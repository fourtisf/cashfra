...
3. **Received vs booked.** `recv(t)`: unpaid→0, dp→`paid`, paid→`usd`. Money-in totals (`gi`) use *received*. Receivables (`pi`) = Σ(usd − recv) on income.
4. **Commission is per-recipient.** In the form, **% is the source of truth** — USD is derived (`deal usd × pct/100`) and shown read-only. Typing a team member's name auto-fills their default %. Multiple recipients per deal are supported.
5. **Commission never double-counts** — the subtlest rule. Two ways to log a commission exist: (a) inside the income entry (`coms[]`, accrual) and (b) as a money-out entry with category **`Team commission`** (payout). Reconciliation is per person name, in `totals()` and `comBook()`:
