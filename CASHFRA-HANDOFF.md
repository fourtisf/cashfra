...
16. **Who brought a listing is `by`, not a commission line.** The `Added by` field on a money-in entry is the truth, and the **Shill team** panel (`shillRows`, `shillHtml`, `oneShillHtml`) is built from it. `broughtList(t)` falls back to the commission recipient **only when there is exactly one** — before the field existed that was the same person, and one recipient is unambiguous where two would be a guess.
    **Two people can bring one listing, and one listing is still one listing.** `by` holds them comma-separated (`+`, `&` and `/` also split; **never a space** — "Ana Maria" is one person far more often than two, and guessing there invents a shiller who does not exist). `broughtList(t)` returns `{name, com, w}` per person, `w` summing to 1: weighted by each one's commission **on that deal** when every one of them has some, and evenly otherwise, because a signal covering half the names is not a signal. Every consumer multiplies by `w` — the leaderboard, the six-period spark, *What they sell*, *Every listing* — so the board can never add up to more money than came in. The listing **count** is deliberately not weighted (they each brought it), which is why `shillHtml` counts deals off the listings rather than summing `o.d`. `broughtAny(t)` asks only whether anybody is credited; `broughtShare(t,name)` is one person's slice. The entry form catches the trap this came from: a single name containing a space whose parts are all in `S.team` gets a hint to write it with a comma, rather than being split behind ALFA's back.
    On the Activity list a credited listing names them in its meta line and carries the commission on its own line as a **subtraction** (`fee −$40.00`, `✓` once every recipient is settled). Both of those are deliberate: `+$148.43` with `fee $22.26` under it reads like two sums the deal earned, and a fee that only appears on settled entries is missing from exactly the deals whose fee is easiest to forget — so `row()` builds a list of sub-lines rather than one, and unpaid and part-paid listings keep theirs alongside the status.
    **Naming somebody in `Added by` fills their commission** from `S.team` (`byComs()`), opens *More details* so the fee is seen, and never removes a row — a row already on screen is the owner's answer. It only ever adds for people who have a default %; anyone else gets a hint saying so, because silently crediting a shiller with no fee against them is the state this whole field kept producing. Two or more, or none, and the deal stays uncredited and is counted in *Not credited to anyone* rather than attributed to somebody who might not have earned it. Anything inferred that way is labelled as such, on the row and in the panel's note; do not quietly drop that label, it is the difference between a record and a guess. The panel ranks by **received**, not booked — a deal that never landed brought nothing in — and is all-time, like Clients, because a shiller's record is not one month.
