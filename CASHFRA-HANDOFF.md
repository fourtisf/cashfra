...
- The *Shill team* panel's **Weekly / Monthly** buttons set the app's own `per` — the same `per`/`S.per`/`save()` the hero's M/W does, and they repaint the hero's buttons too. That is deliberate: rule 7 says one period drives everything visible, and a panel with a private idea of "this week" would be a second one. Only **All time** is panel-local (`shillAll`). If you add another such toggle, do the same rather than forking the period.
