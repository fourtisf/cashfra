...
Entry = {
  ...
  by,                                     // income only: who brought the listing ("Added by")
...
16. **Who brought a listing is `by`, not a commission line.** The `Added by` field on a money-in entry is the truth, and the **Shill team** panel (`shillRows`, `shillHtml`, `oneShillHtml`) is built from it. `broughtList(t)` falls back to the commission recipient **only when there is exactly one** — before the field existed that was the same person, and one recipient is unambiguous where two would be a guess.
    **Two people can bring one listing, and one listing is still one listing.** `by` holds them comma-separated (`+`, `&` and `/` also split; **never a space** — "Ana Maria" is one person far more often than two, and guessing there invents a shiller who does not exist). `broughtList(t)` returns `{name, com, w}` per person, `w` summing to 1: weighted by each one's commission **on that deal** when every one of them has some, and evenly otherwise, because a signal covering half the names is not a signal. Every consumer multiplies by `w` — the leaderboard, the six-period spark, *What they sell*, *Every listing* — so the board can never add up to more money than came in. The listing **count** is deliberately not weighted (they each brought it), which is why `shillHtml` counts deals off the listings rather than summing `o.d`. `broughtAny(t)` asks only whether anybody is credited; `broughtShare(t,name)` is one person's slice. The entry form catches the trap this came from: a single name containing a space whose parts are all in `S.team` gets a hint to write it with a comma, rather than being split behind ALFA's back.
    On the Activity list a credited listing names them in its meta line and carries the commission on its own line as a **subtraction** with the rate it works out at (`fee −$40.00 · 20%`, `✓` once every recipient is settled). The rate is taken off the money (`comv/usd`), never off the `pct` fields, so a deal edited after the fee was agreed shows what that fee costs it *now*. Both of those are deliberate: `+$148.43` with `fee $22.26` under it reads like two sums the deal earned, and a fee that only appears on settled entries is missing from exactly the deals whose fee is easiest to forget — so `row()` builds a list of sub-lines rather than one, and unpaid and part-paid listings keep theirs alongside the status.
    **Naming somebody in `Added by` fills their commission** from `S.team` (`byComs()`), opens *More details* so the fee is seen, and never removes a row — a row already on screen is the owner's answer. **Every** name gets a row — with a default % it arrives filled in, without one it arrives empty and waiting, and the hint says where to type it. Naming somebody and getting nothing back is the state this whole field kept producing, and sending ALFA to Settings first, mid-entry, is not an answer to it. A row saved with a name and no % is harmless: `comv` is 0, so rule 6 and `broughtList`'s inference both ignore it, and the name is still there next time the entry is opened. Two or more, or none, and the deal stays uncredited and is counted in *Not credited to anyone* rather than attributed to somebody who might not have earned it. Anything inferred that way is labelled as such, on the row and in the panel's note; do not quietly drop that label, it is the difference between a record and a guess. The panel ranks by **received**, not booked — a deal that never landed brought nothing in — and is all-time, like Clients, because a shiller's record is not one month.

## Feature inventory (regression checklist)

PIN gate (setup/lock/change/forgot, auto-lock on leaving with a grace period) · brand switcher + inline "New brand" · M/W toggle with 12-bucket strip, ‹›/arrow-key nav, jump-to-period · animated net + growth pills + streak · **four hero chips** (Money in, Money out, Commission, Shill team — 2×2 on a phone, four across above 600px) with delta lines · notices (receivables filter, recurring one-tap log w/ undo) · entry form (brand chips, package chips, centered amount w/ "2.5 bnb" parsing, token pills, auto-rate, chain pills, Team-bonus % quick calc, multi-recipient commission with owed-hint "use this amount", announcement link with Open ↗, status/partial, recurring switch) · edit/delete+undo/duplicate/save-&-add-more · search + filters · Commission panel (netted book, per-person Mark-all-paid, per-recipient per-deal toggles, payouts list) · Insights (**day-by-day cash-flow calendar**, break-even day, biggest-client share, deal-size spread, composition bars with a %-of-spend / %-of-money-in toggle, spending caps vs actual, **period-vs-period comparison** per category with 6-period sparklines, **brand vs brand** table, package & chain mix donuts with a this-period/all-time toggle, package profitability + margin, cost ratio vs prev, avg deal, run-rate) · Clients (net of commission, repeat rate, contacts) · **Shill team** (who brings the listings — Day/Week/Month/All toggle, spelled-out date spans, leaderboard by money received, **Every listing** day-by-day log, tap for their record) · recap ✨ stories · CSV month/all (21 cols incl. Brand, Added by, Announce link) · build number + offline-cache state under Your data · JSON backup/restore + overdue reminder + OS share · invoice/quotation sheet (copy text, print to PDF) · client contacts · **tap a client to open their whole history** (Back returns to the list first) · monthly target card · **tap a person in Commission for their statement** (every deal, effective rate vs their default, share of the pot, six-period trend, copy as text) · **shared costs** — a money-out entry marked *Shared* is carried by every brand, split by each one's share of the money in, with its own Insights section and a line in the brand table · **duplicate-entry warning** before saving (client + amount + coin + date + brand) · **cash-only warning** on a listing that is not settled · **email sign-in** (Settings → Sign in) with a prompt on an empty unsigned device · sample data on request (never seeded) + wipe w/ undo · keyboard: n, /, Enter, Esc, arrows.
