...
- Signing in is email plus a six-digit code, and all it does is fetch the sync token. Keep it that way: the ledger endpoints stay token-only, so the part that was tested does not move when the login does. There is **no sign-up** — `ALLOW` on the server is the whole membership list.
- Every signed-in device holds the **same** token, so there is no per-device revoke. `sync-server.js --rotate <email>` moves the account to a new token **and renames the blob with it**. Never write a revoke that leaves the ledger behind under the old token; that is a lost book.
