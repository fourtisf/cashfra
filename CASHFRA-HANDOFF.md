...
14. **A cashout is not a cost.** `Cashout` (`CASHOUT`, `isCashout()`) is money-out that moves ALFA's own profit to rupiah. The books are exactly as rich afterwards, so it is **excluded from `go` (Money out), from net profit, from the spending mix and from the caps** — the same shape as rule 6's exclusion, for a different reason: a commission expense settles an accrual, a cashout settles nothing at all. It is counted only in `totals().cash`, shown as *taken out* on the running footer and in the **Cash taken out** section, where *still in the book* = all-time net − all-time cashout. It is in `NOT_SHARED`: one transfer was made, out of one brand's money, so it is never split across brands the way a bill is. The day header skips it too — a day you withdrew is not a day you lost. Logging a withdrawal must leave net profit unchanged; that is the invariant.
