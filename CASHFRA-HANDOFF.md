...
9. **Access code.** Preset **162007**, seeded once via marker `lockPreset='162007-v1'` (salt `'fs1'`, hash = SHA-256(`salt:code`), FNV-1a fallback where `crypto.subtle` is unavailable). PIN pad shows when `lockNum`; auto-submits at `lockLen` digits. Change/remove in Settings → App lock. "Forgot" = wipe everything + reseed 162007. This is **client-side deterrence, not security** — the owner knows; don't pretend otherwise, and don't remove it.
