...
16. **Who brought a listing is `by`, not a commission line.** The `Added by` field on a money-in entry is the truth, and the **Shill team** panel (`shillRows`, `shillHtml`, `oneShillHtml`) is built from it. `broughtBy(t)` falls back to the commission recipient **only when there is exactly one** — before the field existed that was the same person, and one recipient is unambiguous where two would be a guess. Two or more, or none, and the deal stays uncredited and is counted in *Not credited to anyone* rather than attributed to somebody who might not have earned it. Anything inferred that way is labelled as such, on the row and in the panel's note; do not quietly drop that label, it is the difference between a record and a guess. The panel ranks by **received**, not booked — a deal that never landed brought nothing in — and is all-time, like Clients, because a shiller's record is not one month.
